...
if options.jlist == '':
    ...
    for jobnum in range(0,options.jobs):
        ...
        joblist.append([jobnum, runtime, arrive])
        print '  Job', jobnum, '( length = ' + str(runtime) + ', arrive = ' + str(arrive) + ' )'
        if (options.policy == 'SJF'):
            arrive += random.randint(0, 2)
else:
    jobnum = 0
    ...
    if options.policy == 'SJF':
        ...
        while not toend:
            ...
            for i in xrange(len(joblist)):
                if joblist[i][2] <= T and joblist[i][1] > 0:
                    if todo == -1:
                        todo = i
                    if joblist[todo][1] > joblist[i][1]:
                        todo = i
                if joblist[i][1] > 0:
                    toend = False
            if toend:
            ...
            if todo == -1:
                ...
                print "Time %d, Jobid %d, Remaining %d. " % (T, joblist[todo][0], joblist[todo][1])
                joblist[todo][1] -= 1
            T += 1
