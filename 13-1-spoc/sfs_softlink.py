...
class fs:
    def __init__(self, numInodes, numData):
        ...
        self.ibitmap.markAllocated(self.ROOT) # allocate
        self.inodes[self.ROOT].setAll('d', 0, 2) # dir addr:0  reference:2
    ...
    # DONE
        ...
        parent_dataBlock.delDirEntry(fileName)
    ...
    # DONE
        ...
        if parent_dataBlock.getFreeEntries() > 0:
            if parent_dataBlock.dirEntryExists(newfile):
                ...
                parent_dataBlock.addDirEntry(newfile, tinum)
        else:
            return -1
        return tinum
            ...
            if parent_dataBlock.dirEntryExists(newfile):
                ...
                if ftype == 'free':
                    ...
                    # if a directory, have to allocate directory block for basic (., ..) info
                        ...
                        new_dataBlock.setType('d')
                        new_dataBlock.addDirEntry('.' , new_inodeNum)
                        new_dataBlock.addDirEntry('..' , self.nameToInum[parent])
                        new_inode.setAll(ftype, new_dataBlockNum, 2)
    ...
    # DONE
        ...
        if curSize == 0:
            ...
            new_dataBlock = self.data[new_dataBlockNum]
            # no data blocks left
            if new_dataBlockNum == -1:
                return -1
            self.inodes[inum].setAddr(new_dataBlockNum)
            # write file data
            new_dataBlock.setType('f')
            new_dataBlock.addData(data)
        else:
            # write file data
            dataBlock = self.data[self.inodes[inum].getAddr()]
