...
class fs:
    ...
    # YOUR CODE, 2013011337
        ...
        self.data[self.inodes[pInum].getAddr()].delDirEntry(tfile)
    ...
    def writeFile(self, tfile, data, itertimes=0):
        if itertimes > 10:
    ...
    # DONE
        ...
        if printOps:
            print 'fd=open("%s", O_WRONLY|O_APPEND); write(fd, "%s", BLOCKSIZE); close(fd);' % (tfile, data)
        return 0

    def createSymLink(self, target, file):
        parent = self.getParent(file)
        pInum = self.nameToInum[parent]
        pDnum = self.inodes[pInum].getAddr()
        # check directory entry num
        if self.data[pDnum].getNumEntries == 0:
            return -1
        # check whether name unique
        if self.data[pDnum].dirEntryExists(file):
            return -1
        # Alloc New file
        nInum = self.inodeAlloc()
        nDnum = self.dataAlloc()
        if nInum < 0 or nDnum < 0:
            return -1
        self.inodes[nInum].setAll('s', nDnum, 1)
        self.data[nDnum].setType('s')
        # do not check whether target exists
        self.data[nDnum].setTarget(target)

        # inc parent ref count and add entry to parent
        self.inodes[pInum].incRefCnt()
        self.data[pDnum].addDirEntry(file, nInum)
    ...
    def run_symlink_test(self):
        print 'Symbolic Link Test Begin:\n'
        print 'Initial state'
        print ''
        self.dump()
        print ''
        ...
        print 'mkdr("/g");'
        self.dirs.append("/g");
        self.nameToInum['/g'] = self.createFile('/', 'g', 'd')
        print ''
        self.dump()
        print ''
        ...
        print 'create("/q");'
        self.files.append("/q");
        self.nameToInum['/q'] = self.createFile('/', 'q', 'f')
        print ''
        self.dump()
        print ''
        ...
        print 'create("/u");'
        self.files.append("/u");
        self.nameToInum['/u'] = self.createFile('/', 'u', 'f')
        print ''
        self.dump()
        print ''
        ...
        print 'symlink("/u", "/v");'
        self.files.append("/v");
        self.nameToInum['/v'] = self.createSymLink('/u', '/v')
        print ''
        self.dump()
        print ''

        print 'fd=open("/v", O_WRONLY|O_APPEND); write(fd, "testtest", BLOCKSIZE); close(fd);'
        self.writeFile('/v', "testtest")
        print ''
        self.dump()
        print ''
        ...
        print 'symlink("/v", "/u");'
        self.files.append("/u");
        self.nameToInum['/u'] = self.createSymLink('/v', '/u')
        print ''
        self.dump()
        print ''

        print 'try write "/v"'
        self.writeFile('/v', "testtest3")
        print ''
        self.dump()
        print ''
